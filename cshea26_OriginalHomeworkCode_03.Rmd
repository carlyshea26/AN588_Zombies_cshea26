...
for (i in 1:100) {
  ...
  sd_height[i] <- sd(s.height)
  sd_weight[i] <- sd(s.weight)
  sd_age[i] <- sd(s.age)
  sd_zombies_killed[i] <- sd(s.zombies_killed)
  sd_years_of_education[i] <- sd(s.years_of_education)
}

#now trying to figure out what the means are of for 100 replicates?
summary_list <- list(
  means_height, means_weight, means_age, means_zombies_killed, means_years_of_education)
lapply(summary_list, summary)

#printing all of the standard deviations for the replicates (i feel like this is kind of a waste of time though)
list(sd_height, sd_weight, sd_age, sd_zombies_killed, sd_years_of_education)

#giving up on looking at the actual data, now looking at the histograms
...
set.seed(123)
s2.height <- replicate(100, s.height) #taking samples that i created with the s."X" label in the question above to replicate for larger replication size
s2.weight <- replicate(100, s.weight)
s2.age <- replicate(100, s.age)
s2.zombies_killed <- replicate(100, s.zombies_killed)
s2.years_of_education <- replicate(100, s.years_of_education)

#putting it in a data frame to make it look cleaner
sampling_means <- data.frame(
  Variable = c("Height", "Weight", "Age", "Zombies Killed", "Years of Education"),
  Mean_of_Sampling_Distribution = c(mean(s2.height), mean(s2.weight), mean(s2.age), mean(s2.zombies_killed), mean(s2.years_of_education)),
  SD_of_Sampling_Distribution = c(sd(s2.height), sd(s2.weight), sd(s2.age), sd(s2.zombies_killed), sd(s2.years_of_education))
)
...
#5.There were so many different parts to answer in the last two questions that I feel like I got lost in answering it. I have no idea how to consense this down into something easier to replicate (maybe a function that I could apply to all the different variables rather than writing each one again).